...
*Slides last updated on **November 3, 2025**. Slides authored by Sabrina Nardin. AI used to polish slides style and fix typos.*
</span>
...
**Conditional statements are like logical tests that let our code make decisions: the code checks conditions, evaluates them, and chooses what to do**
...
4. Vectorized tests with `ifelse()` to apply a condition to each element of a vector (not technically a conditional statement like the previous three, more on this when we get there!)
...
5. Share one question about this code on [Ed Discussion](https://edstem.org/us/courses/86758/discussion)
...
Note the `if... else` evaluates only one single element (here a single number: the mean of `movie_rating`)
...
Use `if...else if...else` when your code needs to test **multiple conditions** and take different actions depending on which condition is `TRUE`. We do this by adding one or more **`else if`** statement(s) between the initial `if` and the final `else`
...
Copy the code below in your console and move the line `temperature >= 10` before the line `temperature >= 20`. Then run the code with a temperature of 25 as input. What do you notice?
...
Once done, [share your code here](https://codeshare.io/G8nxLD){target="_blank"}. If you have questions post on [Ed Discussion](https://edstem.org/us/courses/86758/discussion) or ask us directly!
...
Copy the code below into your console and add another `else if` statement that checks whether the temperature is between 0 (inclusive) and 10 (exclusive). If that condition is true, set weather to "Cold" (so the “Cold” range is 0–9°C).
...
Our final case for today is `ifelse()`: it look like a conditional statement, but it's actually a function that applies a condition to **each element of a vector**. 
...
- If the input of `ifelse()` is a **vector**, the output is also a **vector**: `ifelse()` evaluates **each element of the vector**, applying the specified action for each element.

- This is possible because **`ifelse()` supports vectorized operations**: operations directly applied on entire vectors, rather than looping through individual elements one-by-one.
...
# if / else: works for a single value only
if (x > 10) "High" else "Low"

# ifelse(): works element-wise (vectorized)
...
For this reason, `ifelse()` is often used to make changes to columns in a data frame. For example, what is the output of this code?
...
If you have multiple conditions (more than two): use separate `mutate()` + `ifelse()` steps (like on this slide) rather than nesting them (like in the previous slide).
:::
...
People often use `ifelse()` to recode variables in a dataframe. It works when the recoding can be done with a single `ifelse()`, as shown in this example. Though I recommend using `case_when()` for recoding variables (cleaner, more beginner friendly), as we learned in the recording variables module.

Example: the variable `decisionDirection` takes four values (1 conservative, 2 liberal, 3 other, and NA). Recode it to take three values (0 conservative, 1 liberal, and NA for both NA and other)
...
- We learned vectorized tests with the function `ifelse()` and how this differs from standard conditional statements
