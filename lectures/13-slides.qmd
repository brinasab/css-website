...
*Slides last updated on **November 10, 2025**. Slides authored by Sabrina Nardin. AI used to polish slides style and fix typos.*
</span>
...
#### `output` is a vector where we store the results of the loop:    
- To access elements of a vector, we only need single square brackets `[]`.  
...
- To access a dataframe column as a vector, we need double brackets `[[]]`.  
- So `penguins[[i]]` extracts the *i-th column* of the dataframe, and turns it into a vector. 
...
```{r echo=TRUE, results="hide"}
is.vector(penguins[[1]])   # TRUE (the raw vector)
is.data.frame(penguins[1]) # TRUE (a 1-col tibble/data frame)

mean(penguins[[1]], na.rm = TRUE)  # works, correct type (vector)
mean(penguins[1], na.rm = TRUE)    # error, wrong type (1-col tibble)
```
...
**With vectors:** `[]` and `[[ ]]` behave the same, but it's still good practice to consistently use `[]` when subsetting vectors and `[[ ]]` when extracting columns from data frames.
...
But besides **using** functions created by others, we can also **write our own functions!**
