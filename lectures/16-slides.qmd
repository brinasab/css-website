...
2. Expanding Last Lecture’s Scraper
...
- What’s behind a webpage (read and navigate HTML structure)
...
# 2. Expanding Last Lecture’s Scraper {.slide .center .middle}
...
- **Bad formatting:** websites are *supposed* to use a clean, logical, hierarchical HTML structure…  but many don’t, which makes scraping a bit of an art.

- **Change over time:** websites update their layout, so old code may break. The good news: fixes are usually small.

- **Limits:** some sites cap requests (e.g., only 50 pages or 2,500 items), and after that they stop responding. You’ll need to scrape in smaller “chunks.”
...
- A page is dynamic when it uses **JavaScript** to load content after the initial HTML. That content won’t appear in the raw HTML from `read_html()`, which returns empty or incomplete results.
...
- If an API requires a username, password, or private key, you’ll need to supply the same credentials when using its corresponding R wrapper package.
